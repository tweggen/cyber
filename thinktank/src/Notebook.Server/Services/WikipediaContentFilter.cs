...
public sealed partial class WikipediaContentFilter : IContentFilter
{
    ...
    public FilterResult Filter(string content)
    {
        ...
        // 1. Strip [edit] link text
        result = EditLinkPattern().Replace(result, "");

        // 2. Remove citation brackets: [1], [2], [12], [citation needed], etc.
        result = CitationBracketPattern().Replace(result, "");

        // 3-6. Remove boilerplate sections (See also, References, External links, Further reading, Notes)
        ...
        // 7. Remove Category: link lines
        result = CategoryLinePattern().Replace(result, "");

        // 8. Remove navigation footer patterns (navbox remnants, "This article..." tables)
        result = NavboxPattern().Replace(result, "");

        // 9. Collapse resulting blank lines (3+ newlines → 2)
